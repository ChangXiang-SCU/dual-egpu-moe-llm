...
It works: **17.7-19.6 tok/s decode at the model's full native 256k context**, up from 8.2 tok/s
with the same two cards and no expert cache.

Then the same machine ran a **27B dense** model at 6-bit and beat it on every axis — 24x on
prefill. **[That comparison is the most useful page here](docs/dense-vs-moe.md)**, and it is the
thing to read first if you are deciding what to run on a 32 GB machine rather than reproducing this
particular stunt.

Also worth someone else's time:
...
4. **[The measurements](docs/benchmarks.md)**, including the parts that did not work and the
   correctness limits of the cache path.
...
| part | size | behaviour |
|---|---|---|
| `ffn_*_exps` — 512 experts x 48 layers | **46.1 GB** | 10 of 512 picked per token per layer; over a few hundred tokens essentially all of them get touched, so this is the hot set |
| `per_layer_token_embd.weight` — the 51B n-gram table | **28.8 GB** | 320 M rows x 160 dims; a few rows looked up per token. mmap keeps only the touched pages resident — tens of MB in practice |
...
VRAM holds a *copy*, not a *move*: the authoritative expert weights must stay in RAM because the
LRU cache evicts, and an evicted expert has to be readable again. So the capacity is still 46 GB —
the 23 GB in VRAM is acceleration, not storage. Statically partitioning instead (half the experts
pinned on the GPUs with `-ot`, the rest on the CPU) was tried and gives **6.1 tok/s**: expert
selection is near-uniform in the long run, so a static half serves about half the accesses, while
an LRU cache of similar size serves 93 %. What matters is not how much you can hold but what share
of accesses land on the GPU.
...
Qwen3.8-Flash-Next UD-Q2_K_XL, 2x RX 6950XT (OCuLink Gen4 x4 each), 7840U, 64 GB, Vulkan, decode
...
Prefill is the wall: **~18 tok/s**, because batches larger than `LLAMA_MOE_CACHE_MAX_TOKENS` skip
the cache and every expert is computed on the 7840U. Letting prefill use the cache is worth +75 %
(30.5 tok/s on a 3722-token prompt) but is **numerically broken above batch 16** for two separate
reasons, both documented in [docs/benchmarks.md](docs/benchmarks.md). It is reported upstream rather
than shipped. Every configuration here was checked with greedy decoding against output hashes, not
just eyeballed for plausibility.

That 18 tok/s prefill is what the [dense comparison](docs/dense-vs-moe.md) is about: a 27B that
fits in VRAM does the same work at 403-444 tok/s.
...
- The built-in display runs off the 780M iGPU (279 MiB), leaving both discrete cards entirely to
  compute. `--list-devices` reports a static heap budget, not live usage — read
  `\GPU Adapter Memory(*)\Dedicated Usage` instead.
